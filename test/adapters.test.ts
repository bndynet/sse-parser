    ...
    const events = await collect(
      anthropicStream(
        sseResponse([
          ...
          sse({ type: 'message_delta', delta: { stop_reason: 'end_turn' }, usage: { output_tokens: 7 } }, 'message_delta'),
    ...
    const events = await collect(
      geminiStream(
        sseResponse([
          ...
          sse({ candidates: [{ content: { parts: [{ functionCall: { name: 'a', args: {} } }, { functionCall: { name: 'b', args: {} } }] }, finishReason: 'STOP' }], usageMetadata: { promptTokenCount: 1, candidatesTokenCount: 2, totalTokenCount: 3 } }),
        ]),
      ),
    );
...
describe('openaiResponsesStream', () => {
  it('maps text/reasoning/tool-call/usage from Responses events', async () => {
    const events = await collect(
      openaiResponsesStream(
        sseResponse([
          ...
          sse({ type: 'response.completed', response: { usage: { input_tokens: 4, output_tokens: 6, total_tokens: 10 } } }, 'response.completed'),
        ]),
      ),
    );
    ...
    const dones = events.filter((e) => e.type === 'done');
    expect(dones).toHaveLength(1);
    expect((dones[0] as any).usage).toEqual({ promptTokens: 4, completionTokens: 6, totalTokens: 10 });
  });
});
